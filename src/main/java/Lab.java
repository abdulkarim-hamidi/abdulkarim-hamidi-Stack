...
/**
 * Stacks are LIFO - last in, first out, like a stack of plates or books, or the puzzle game 
 "Towers of Hanoi".
 * They are used in algorithms where it is needed to keep track of the ordering in which the 
 elements were inserted.
 *
 * ArrayDeque fills the role of *both* queues and stacks. You could think of a Deque as a deck 
 *  cards (pronounced the
 ...
 * And you can read about the stack class here (deque exhibits the same behavior as Stack and 
 * contains the same
 * methods): https://docs.oracle.com/javase/7/docs/api/java/util/Stack.html
 */
public class Lab {
    /**
     * Implement and return some class that implements Deque, such as ArrayDeque, which contains
     *  all the functionality
     * of a Stack. This challenge will not be using the Stack class, because the Stack class is 
     * known to have a shoddy
     * implementation as it hasn't been changed since the very earliest version of Java. Deques 
     * were made to exhibit
